...
(`RAG_API_URL`), and the RAG API's generation layer is pluggable (`nile_chat_ollama` by
default — the fine-tuned GGUF served via Ollama; `nile_chat_gguf` for in-process
llama-cpp, and Claude available as a comparison/fallback). This lets each engineer build
and test their component independently.
